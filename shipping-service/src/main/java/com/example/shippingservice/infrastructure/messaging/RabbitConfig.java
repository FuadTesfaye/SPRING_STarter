...
public class RabbitConfig {
    ...
    public static final String DLQ = "shipping.queue.dlq";
...
Binding binding0(Queue serviceQueue, TopicExchange topicExchange) {
    return BindingBuilder.bind(serviceQueue).to(topicExchange).with("payment.completed");
}

@Bean
Binding binding1(Queue serviceQueue, TopicExchange topicExchange) {
    return BindingBuilder.bind(serviceQueue).to(topicExchange).with("stock.reserved");
}
