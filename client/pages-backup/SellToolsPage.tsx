...
const TOOL_TYPES = ['Power Tools', 'Hand Tools', 'Garden Tools', 'Automotive Tools', 'Woodworking Tools', 'Measuring Tools', 'Safety Equipment', 'Cleaning Tools', 'Electrical Tools', 'Plumbing Tools', 'Painting Tools', 'Construction Tools', 'Storage & Organization', 'Workshop Equipment'];
const BRANDS = ['DeWalt', 'Milwaukee', 'Makita', 'Bosch', 'Ryobi', 'Craftsman', 'Black & Decker', 'Porter-Cable', 'Stanley', 'Husky', 'Klein Tools', 'Channellock', 'Irwin', 'Vise-Grip', 'Klein', 'Wera', 'Wiha', 'Snap-on', 'Matco', 'Cornwell', 'Proto', 'SK'];
const CONDITIONS = ['New', 'Like New', 'Excellent', 'Good', 'Fair', 'Used', 'Refurbished', 'Vintage', 'Antique', 'Professional Grade', 'Industrial Grade'];
const POWER_TYPES = ['Corded Electric', 'Cordless Battery', 'Pneumatic', 'Manual', 'Hydraulic', 'Gas Powered', 'Solar Powered', 'Dual Power', 'Multi-voltage'];
const SIZES = ['Compact', 'Small', 'Medium', 'Large', 'Extra Large', 'Professional', 'Industrial', 'Mini', 'Handheld', 'Portable', 'Stationary', 'Adjustable'];

export default function SellToolsPage() {
  const navigate = useNavigate();
  const { addProduct, isLoading, error, clearError } = useProductStore();
  const { isAuthenticated } = useAuthStore();
  
  const [formData, setFormData] = useState<ToolsFormData>({
    name: '',
    description: '',
    price: '',
    comparePrice: '',
  ...
  const [formErrors, setFormErrors] = useState<Partial<ToolsFormData>>({});
  ...
  const validateForm = (): boolean => {
    const errors: Partial<ToolsFormData> = {};
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (formErrors[name as keyof ToolsFormData]) {
      setFormErrors(prev => ({
        ...prev,
        [name]: undefined
      }));
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          ...
          <div className="border-b border-gray-200 p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Sell Tools</h1>
                <p className="text-gray-600">List your tools and equipment</p>
              </div>
            </div>
          </div>
          ...
          {submitSuccess && (
            <div className="m-6 p-4 bg-green-50 border border-green-200 rounded-lg flex items-center gap-3">
              ...
              <span className="text-green-700">Tool added successfully!</span>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Describe the tool, condition, features, usage, etc."
                maxLength={1000}
              />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={handleInputChange}
                    className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${
                      formErrors.toolType ? 'border-red-500' : 'border-gray-300'
                    }`}
                  >
                  ...
                  {formErrors.toolType && <p className="mt-1 text-sm text-red-600">{formErrors.toolType}</p>}
                </div>
                ...
                <div>
                  ...
                  {formErrors.powerType && <p className="mt-1 text-sm text-red-600">{formErrors.powerType}</p>}
                </div>
            ...
            <div>
              <label className="flex items-center gap-2 text-sm font-medium text-gray-700">
                ...
                />
                Feature this tool on homepage
              </label>
              <p className="mt-1 text-sm text-gray-500">Featured tools appear in trending section</p>
            </div>
