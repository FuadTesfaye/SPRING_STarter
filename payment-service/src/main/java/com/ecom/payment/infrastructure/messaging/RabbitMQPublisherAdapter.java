...
public class RabbitMQPublisherAdapter implements EventPublisher {
    ...
    public void publishPaymentProcessed(PaymentProcessedEvent event) {
        CompletableFuture.runAsync(() -> {
            try {
                // 1. Notify shipping service if completed
                if ("COMPLETED".equalsIgnoreCase(event.status())) {
                    Map<String, Object> shippingPayload = Map.of("orderId", event.orderId());
                    try {
                        restTemplate.postForObject(shippingServiceUrl + "/api/events/payment-completed", shippingPayload, String.class);
                    } catch (Exception ex) {
                        System.err.println("[payment-service] Failed to call shipping-service: " + ex.getMessage());
                    }
                }

                // 2. Notify notification service
                String routingKey = event.status().equalsIgnoreCase("COMPLETED") ? "payment.completed" : "payment.failed";
                ...
                try {
                    ...
                    System.err.println("[payment-service] Failed to call notification-service: " + ex.getMessage());
                }
            } catch (Exception ex) {
                System.err.println("[payment-service] Error processing events: " + ex.getMessage());
            }
        });
    }
}
